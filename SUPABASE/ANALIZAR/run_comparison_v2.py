...
def run_comparison():
    ...
    for row in excel_data:
        if len(row) > 1 and isinstance(row[1], str):
            sigla = row[1]
            excel_siglas.append(sigla)
            if sigla not in js_set:
                missing.append(f"{sigla} - {row[2]} ({row[3]})")
    
    with open('discrepancias.txt', 'w', encoding='utf-8') as f:
        f.write("=== MATERIAS EN EXCEL PERO NO EN DATA.JS ===\n")
        if not missing:
            f.write("Ninguna. Todas las siglas del Excel están en data.js.\n")
        else:
            for m in missing:
                f.write(f"- {m}\n")
        
        f.write("\n=== MATERIAS EN DATA.JS PERO NO EN EXCEL ===\n")
        excel_set = set(excel_siglas)
        extra_in_js = [s for s in data_js_siglas if s not in excel_set]
        if not extra_in_js:
            f.write("Ninguna.\n")
        else:
            for s in extra_in_js:
                f.write(f"- {s}\n")
